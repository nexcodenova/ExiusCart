...
def tiktok_authorize(
    ...
    # TikTok Shop is a "marketplace" channel in the shared channel-slot pool
    # now — subject to Launch's 1-per-category cap like everything else, not
    # its own Launch-or-better-only gate. TikTok is OAuth-based so it has its
    # own authorize endpoint (like eBay/Daraz) rather than going through
    # channels.py's connect_channel().
    check_channel_slot(shop_id, db, "tiktok", plan_type)
