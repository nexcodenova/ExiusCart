...
class ManualCategoryIn(BaseModel):
    name: str


@router.post("/shops/{shop_id}/channels/{channel_id}/categories", status_code=201)
def add_manual_category(
    shop_id: int,
    channel_id: int,
    data: ManualCategoryIn,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Manually add a category — used when the channel hasn't implemented a categories API."""
    _shop_or_404(shop_id, current_user, db)
    conn = db.query(ChannelConnection).filter(
        ChannelConnection.id == channel_id,
        ChannelConnection.shop_id == shop_id,
    ...
    existing = db.query(ChannelCategory).filter(
        ChannelCategory.channel_connection_id == channel_id,
        ChannelCategory.channel_category_id == slug,
    ).first()
    if existing:
    ...
    cat = ChannelCategory(
        channel_connection_id=channel_id,
        channel_category_id=slug,
        name=data.name.strip(),
    )
    db.add(cat)
    db.commit()
    db.refresh(cat)
    return {"id": cat.channel_category_id, "name": cat.name}


@router.delete("/shops/{shop_id}/channels/{channel_id}/categories/{category_id:path}", status_code=200)
def delete_manual_category(
    shop_id: int,
    channel_id: int,
    category_id: str,
    db: Session = Depends(get_db),
    ...
    """Delete a manually added channel category."""
    _shop_or_404(shop_id, current_user, db)
    cat = db.query(ChannelCategory).filter(
        ChannelCategory.channel_connection_id == channel_id,
        ChannelCategory.channel_category_id == category_id,
    ).first()
    if not cat:
