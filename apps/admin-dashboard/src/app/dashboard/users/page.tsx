...
import {
  Search, Filter, User, Shield, Store,
  CheckCircle, XCircle, Loader2, Ban,
} from 'lucide-react';
...
export default function UsersPage() {
  ...
  const filtered = statusFilter === 'all'
    ? users
    : users.filter((u) => statusFilter === 'active' ? u.is_active : !u.is_active);
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-[#151F32] rounded-xl border border-gray-800 p-4">
          ...
          <p className="text-2xl font-bold text-green-400 mt-1">{loading ? '—' : users.filter((u) => u.is_active).length}</p>
        </div>
        <div className="bg-[#151F32] rounded-xl border border-gray-800 p-4">
          <p className="text-gray-400 text-sm">Suspended</p>
          <p className="text-2xl font-bold text-red-400 mt-1">{loading ? '—' : users.filter((u) => !u.is_active).length}</p>
        </div>
      </div>
                      ...
                      <td className="px-6 py-4">
                        {user.shop_name ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-white">{user.shop_name}</span>
                          </div>
                        ) : (
                          <span className="text-gray-600 text-sm">No shop</span>
                        )}
                      </td>
                      <td className="px-6 py-4">
