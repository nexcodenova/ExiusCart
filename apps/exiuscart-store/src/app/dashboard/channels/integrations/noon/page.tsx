...
import {
  ArrowLeft, ShoppingBag, Loader2, CheckCircle2, Warehouse, RefreshCw, AlertCircle, KeyRound, ListChecks, FileText, Link2Off, LifeBuoy,
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <ChannelListingActivity shopId={shopId} channelType="noon" />
        </div>
      ) : isTheDersiUser ? (
        <TheDersiRestrictionNotice channelKey="noon" />
      ) : (
        <BeforeConnectLayout
          accentClass="bg-yellow-500/10 text-yellow-600 dark:text-yellow-400"
          badges={[
            { icon: KeyRound, label: 'Service account key', desc: 'Your own Noon Partners credentials — you generate and control them' },
            { icon: Warehouse, label: 'Warehouse required', desc: "Pick or set up a warehouse right after connecting, so stock can sync" },
            { icon: FileText, label: 'Manage in Channel Orders', desc: 'Noon orders show up alongside every other channel' },
            { icon: Link2Off, label: 'Disconnect anytime', desc: 'Your Noon listings stay untouched' },
          ]}
          sidebar={<>
            ...
            <SidebarCard icon={ShoppingBag} iconClass="bg-yellow-500/10 text-yellow-600 dark:text-yellow-400"
              title="What syncs" desc="Assign a product to Noon from Channel Categories to list it — every attempt, success or failure, is tracked in Channel Listings." />
          </>}
          steps={[
            { icon: KeyRound, title: '1. Add a service account', desc: 'Noon Partners → API Users.' },
            { icon: ListChecks, title: '2. Paste the 4 credentials', desc: 'From the downloaded credentials JSON.' },
            { icon: Warehouse, title: '3. Choose a warehouse', desc: 'Required before stock can sync.' },
