...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} className="flex items-center gap-1 px-2 py-1 text-xs text-muted-foreground hover:text-primary border border-border rounded">
...
function ResultCard({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="bg-muted/50 border border-border rounded-lg p-3">
      ...
      <p className="text-xs text-muted-foreground mt-1">{value.length} chars</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">AI SEO Tools</h1>
          <p className="text-sm text-muted-foreground">Generate SEO-optimised content, keywords, and meta tags powered by Claude AI</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1 bg-muted rounded-xl p-1">
        {TABS.map(t => (
          <button key={t.id} onClick={() => { setTab(t.id); setResult(null); setError(''); }}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${tab === t.id ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border p-6 space-y-4">
          <h2 className="font-semibold text-foreground text-sm uppercase tracking-wide">Input</h2>
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <input value={descForm[f.key as keyof typeof descForm]} onChange={e => setDescForm(v => ({ ...v, [f.key]: e.target.value }))}
                    className="w-full px-3 py-2 border border-border rounded-lg bg-muted text-foreground text-sm focus:ring-2 focus:ring-primary outline-none"
          ...
          {tab === 'blog' && (
            <>
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Target Word Count</label>
          ...
          {tab === 'audit' && (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">Scan all your products for SEO issues — missing descriptions, short titles, no images, and more.</p>
              ...
              <button onClick={runAudit} disabled={auditLoading}
                className="w-full py-2.5 bg-purple-500 text-white rounded-lg hover:bg-purple-600 disabled:opacity-50 text-sm font-medium flex items-center justify-center gap-2">
        ...
        <div className="bg-card rounded-xl border border-border p-6">
          <h2 className="font-semibold text-foreground text-sm uppercase tracking-wide mb-4">Results</h2>
          ...
          {tab === 'description' && result && (
            <div className="space-y-3">
              ...
              <div className="bg-muted/50 border border-border rounded-lg p-3">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Product Description</span>
              ...
              <div className="bg-muted/50 border border-border rounded-lg p-3">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Keywords / Tags</p>
                <div className="flex flex-wrap gap-1.5">
                  {(result.keywords || []).map((k: string, i: number) => (
                    <span key={i} className="px-2 py-0.5 bg-purple-500/10 text-purple-600 dark:text-purple-400 rounded-full text-xs">{k}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          {tab === 'keywords' && result && (
            <div className="space-y-3">
              <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-3">
                ...
                <p className="font-bold text-foreground">{result.primary_keyword}</p>
              </div>
              {[
                ...
                <div key={g.label} className="bg-muted/50 border border-border rounded-lg p-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {g.data.map((k: string, i: number) => (
                      <span key={i} className="px-2 py-0.5 bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-full text-xs">{k}</span>
                    ))}
                  </div>
                </div>
              ))}
                  ...
                  <div className="space-y-2">
                    {result.content_ideas.map((idea: any, i: number) => (
                      <div key={i} className="flex items-start gap-2">
                        <span className="text-xs bg-muted text-muted-foreground px-1.5 py-0.5 rounded capitalize">{idea.type}</span>
          ...
          {tab === 'blog' && result && (
            <div className="space-y-3">
              ...
              <div className="bg-muted/50 border border-border rounded-lg p-3 max-h-72 overflow-y-auto">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Full Blog Post</span>
                  <CopyBtn text={result.content} />
                </div>
                <pre className="text-sm text-foreground whitespace-pre-wrap font-sans leading-relaxed">{result.content}</pre>
              </div>
              {result.tags?.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {result.tags.map((t: string, i: number) => <span key={i} className="px-2 py-0.5 bg-purple-500/10 text-purple-600 dark:text-purple-400 rounded-full text-xs">{t}</span>)}
                </div>
              )}
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { label: 'Total Products', value: auditData.total_products, color: 'text-foreground' },
                  ...
                  <div key={s.label} className="bg-muted/50 border border-border rounded-lg p-3">
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {auditData.products.map((p: any) => (
                  <div key={p.product_id} className="border border-border rounded-lg overflow-hidden">
                    <button onClick={() => setExpandedProduct(expandedProduct === p.product_id ? null : p.product_id)}
                      ...
                      <span className="flex-1 text-sm font-medium text-foreground truncate">{p.product_name}</span>
                      ...
                      {expandedProduct === p.product_id ? <ChevronUp className="w-4 h-4 text-muted-foreground" /> : <ChevronDown className="w-4 h-4 text-muted-foreground" />}
                    </button>
                      ...
                      <div className="px-3 pb-3 pt-0 border-t border-border bg-muted/30">
                        <ul className="space-y-1 mt-2">
                          {p.issues.map((issue: string, i: number) => (
                            <li key={i} className="flex items-start gap-1.5 text-xs text-orange-600 dark:text-orange-400">
          ...
          {!result && !auditData && !loading && !auditLoading && (
            <div className="flex flex-col items-center justify-center h-48 text-center">
              ...
              <p className="text-sm text-muted-foreground">Fill in the form and click Generate to see AI results here</p>
            </div>
          )}
        </div>
      </div>
