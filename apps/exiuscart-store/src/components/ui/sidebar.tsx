// shadcn/ui Sidebar — adapted from the current shadcn source for this
// project's Tailwind v3.4 (the published version targets v4): every
// `w-(--foo)` parenthesis arbitrary-value became `w-[--foo]` bracket
// syntax (v4-only sugar for the same `var(--foo)` result — v3 needs the
// brackets), `outline-hidden` (a real v4 utility) became `outline-none`,
// and the `floating`/`inset` sidebar variants were dropped entirely
// (they lean on v4's `--spacing()` theme function in a calc() that has
// no v3 equivalent) — this app only ever uses the plain default variant,
// so nothing is lost by not carrying that branch over. `Slot` uses this
// project's already-installed `@radix-ui/react-slot` directly rather
// than the v4 registry's unified `radix-ui` package re-export.
'use client';
...
function Sidebar({
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* Reserves the layout gap on desktop — a real block, not fixed
          positioning, so sibling content (SidebarInset) never needs
          manual padding-left math tied to collapse state. */}
      <div
