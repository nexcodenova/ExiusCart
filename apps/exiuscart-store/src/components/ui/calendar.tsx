// Tremor Calendar [v1.0.0] — copied in from tremor.so (Tremor Raw), same
// adaptation approach as components/charts/*: `cx` -> this project's `cn`,
// @remixicon/react's arrow icons -> lucide-react (already the only icon
// library used everywhere else in this app), and Tailwind v4-only syntax
// converted to this project's v3.4 syntax — `outline-hidden` isn't a v3
// utility (dropped, focusRing already handles the focus-visible state),
// and v4's trailing-`!` important modifier (`text-gray-300!`) rewritten
// to v3's leading-`!` form (`!text-gray-300`).
'use client';
  ...
  ({ onClick, icon, disabled, ...props }: NavigationButtonProps, forwardedRef) => {
    ...
    return (
      <button
        ...
        disabled={disabled}
        className={cn(
          'flex size-8 shrink-0 select-none items-center justify-center rounded-sm border p-1 outline-none transition sm:size-[30px]',
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        day: cn('size-9 rounded-sm text-sm focus:z-10', 'text-gray-900 dark:text-gray-50', 'hover:bg-gray-200 dark:hover:bg-gray-700', focusRing),
        day_today: 'font-semibold',
        day_selected: cn('rounded-sm', 'aria-selected:bg-blue-500 aria-selected:text-white', 'dark:aria-selected:bg-blue-500 dark:aria-selected:text-white'),
        ...
        day_range_middle: cn('!rounded-none', 'aria-selected:!bg-gray-100 aria-selected:!text-gray-900', 'dark:aria-selected:!bg-gray-900 dark:aria-selected:!text-gray-50'),
      ...
      components={{
        ...
        Caption: ({ ...props }) => {
          ...
          const goToPreviousYear = () => {
            ...
            if (previousMonth && (!fromDate || targetMonth.getTime() >= fromDate.getTime())) goToMonth(targetMonth);
          };
          const goToNextYear = () => {
            const targetMonth = addYears(currentMonth, 1);
            if (nextMonth && (!toDate || targetMonth.getTime() <= toDate.getTime())) goToMonth(targetMonth);
          };
              ...
              <div className="flex items-center gap-1">
                {enableYearNavigation && !hidePreviousButton && (
                  <NavigationButton
                    ...
                    aria-label="Go to previous year" onClick={goToPreviousYear} icon={ChevronsLeft}
                  />
                )}
                {!hidePreviousButton && (
                  <NavigationButton
                    disabled={disableNavigation || !previousMonth}
                    aria-label="Go to previous month" onClick={() => previousMonth && goToMonth(previousMonth)} icon={ChevronLeft}
                  />
                )}
              </div>
              ...
              <div className="flex items-center gap-1">
                {!hideNextButton && (
                  <NavigationButton
                    ...
                    aria-label="Go to next month" onClick={() => nextMonth && goToMonth(nextMonth)} icon={ChevronRight}
                  />
                )}
                {enableYearNavigation && !hideNextButton && (
                  <NavigationButton
                    ...
                    aria-label="Go to next year" onClick={goToNextYear} icon={ChevronsRight}
                  />
                )}
              </div>
            </div>
        ...
        Day: ({ date, displayMonth }: DayProps) => {
          ...
          const { activeModifiers, buttonProps, divProps, isButton, isHidden } = useDayRender(date, displayMonth, buttonRef);
