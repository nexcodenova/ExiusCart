...
// Placeholder colored icon chips until real brand logos are dropped in
// (per feedback — logos to follow) — same icon/color choices already used
// on each channel's own card, so nothing here contradicts those.
const SOURCE_NODES = [
  { label: 'Shopify', icon: ShoppingBag, className: 'bg-[#96BF48]/10 text-[#96BF48]' },
  { label: 'Amazon', icon: Package, className: 'bg-orange-400/10 text-orange-500' },
  { label: 'eBay', icon: Tag, className: 'bg-[#E53238]/10 text-[#E53238]' },
  { label: 'Etsy', icon: ShoppingBag, className: 'bg-orange-600/10 text-orange-600' },
  { label: 'TikTok Shop', icon: Music2, className: 'bg-foreground/10 text-foreground' },
  { label: 'Noon', icon: ShoppingBag, className: 'bg-yellow-500/10 text-yellow-600' },
  { label: 'Daraz', icon: ShoppingBag, className: 'bg-orange-500/10 text-orange-500' },
];
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:flex md:flex-col gap-2.5">
            {SOURCE_NODES.map((node) => (
              <div key={node.label} className="flex items-center gap-2 px-3 py-2 rounded-xl border border-border bg-card">
                <span className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${node.className}`}>
