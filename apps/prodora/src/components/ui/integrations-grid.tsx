...
export function IntegrationsGrid() {
  ...
  return (
    <>
      ...
      {/* On mobile, stacked vertically at full card width/normal 4:5 ratio
          (was two tilted cards squeezed to 40vw each side by side — too
          small to actually read, same fix applied to exiuscart-website's
          copy of this component). Tilted "leaning pair" only from sm+. */}
      ...
      <div ref={wrapperRef} style={{ height: `calc(100vh + ${maxTranslate}px)` }}>
        {/* top-20/h-[calc(100vh-5rem)] instead of top-0/h-screen (what
            exiuscart-website uses) — Prodora's Navbar (h-20, sticky top-0
            z-50) stays visible all the time, unlike exiuscart-website's
            (which auto-hides on scroll-down), so pinning this track to
            the true viewport top there put it right underneath the
            navbar, clipping the card titles. Offsetting by the navbar's
            real height fixes it without touching the navbar itself. */}
