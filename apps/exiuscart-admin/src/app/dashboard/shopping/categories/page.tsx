  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-gray-800 bg-[#0F1829] py-16 text-center">
          <Tag className="mx-auto h-8 w-8 text-gray-600" />
          <p className="mt-3 font-medium text-white">No categories yet</p>
          <p className="mt-1 text-sm text-gray-500">Add one, upload its image, then assign products to it.</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {categories.map((c) => (
            <div key={c.id} className="overflow-hidden rounded-xl border border-gray-800 bg-[#0F1829]">
              <div className="relative flex aspect-[4/3] items-center justify-center bg-[#0B1121]">
                {c.image_url ? (
                ...
                {!c.image_url && (
                  <span className="absolute left-2 top-2 rounded bg-amber-500/15 px-2 py-0.5 text-[11px] font-medium text-amber-400">Hidden from Marketplace</span>
                )}
              </div>
              <div className="flex items-center justify-between gap-2 p-4">
                ...
                <div className="flex shrink-0 gap-1">
                  <button onClick={() => openEdit(c)} aria-label={`Edit ${c.name}`} className="rounded-lg p-2 text-gray-400 hover:bg-[#151F32] hover:text-white"><Edit2 className="h-4 w-4" /></button>
                  <button onClick={() => remove(c)} aria-label={`Delete ${c.name}`} className="rounded-lg p-2 text-gray-400 hover:bg-red-500/10 hover:text-red-400"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            </div>
          ))}
      ...
      {showForm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4" onClick={() => setShowForm(false)}>
          <form onSubmit={save} onClick={(e) => e.stopPropagation()} className="w-full max-w-md space-y-4 rounded-2xl border border-gray-800 bg-[#0F1829] p-6">
            <div className="flex items-start justify-between">
              <h2 className="text-lg font-semibold text-white">{editing ? 'Edit category' : 'Add category'}</h2>
              <button type="button" onClick={() => setShowForm(false)} className="text-gray-500 hover:text-white"><X className="h-5 w-5" /></button>
            </div>

            <div>
              <label htmlFor="cat-name" className="mb-1 block text-xs font-medium text-gray-400">Name</label>
              <input
                id="cat-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Pets"
                className="w-full rounded-lg border border-gray-700 bg-[#0B1121] px-3 py-2.5 text-sm text-white placeholder-gray-600 outline-none focus:border-[#6B3FD9]"
              />
            </div>

            <div>
              <p className="mb-1 text-xs font-medium text-gray-400">Tile image</p>
              <div className="flex items-center gap-3">
                <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-gray-700 bg-[#0B1121]">
                  {imageUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                ...
                <div className="space-y-2">
                  <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-gray-700 px-3 py-2 text-sm text-gray-300 hover:bg-[#151F32]">
                    {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}
                    {imageUrl ? 'Replace image' : 'Upload image'}
                    <input type="file" accept="image/*" className="hidden" onChange={(e) => onFile(e.target.files?.[0])} />
                  </label>
                  {imageUrl && (
                    <button type="button" onClick={() => setImageUrl('')} className="block text-xs text-gray-500 hover:text-red-400">Remove image</button>
                  )}
                </div>
              </div>
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button type="button" onClick={() => setShowForm(false)} className="rounded-lg px-4 py-2 text-sm text-gray-400 hover:bg-[#151F32]">Cancel</button>
