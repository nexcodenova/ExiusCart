...
function SourceBadge({ source }: { source: 'manual' | 'lemon_squeezy' }) {
  return source === 'lemon_squeezy' ? (
    <span className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full bg-yellow-500/10 text-yellow-600 font-medium">
      <Zap className="w-3 h-3" /> Lemon Squeezy
    </span>
  ) : (
    <span className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full bg-gray-500/10 text-gray-600 font-medium">
...
export default function PaymentsPage() {
  ...
  const totalRevenue = subs.filter((s) => s.status === 'active').reduce((sum, s) => sum + (s.amount_usd ?? s.amount_paid), 0);
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-600 text-sm mt-1">Subscription payments and approvals</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-gray-600 text-sm">Total Revenue</p><p className="text-xl font-bold text-gray-900 mt-1">${totalRevenue.toLocaleString()}</p></div>
            <div className="p-2.5 rounded-lg bg-[#6B3FD9]/10 text-[#6B3FD9]"><TrendingUp className="w-5 h-5" /></div>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-gray-600 text-sm">Active</p><p className="text-2xl font-bold text-gray-900 mt-1">{subs.filter((s) => s.status === 'active').length}</p></div>
            <div className="p-2.5 rounded-lg bg-green-500/10 text-green-600"><CheckCircle className="w-5 h-5" /></div>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-gray-600 text-sm">Pending</p><p className="text-2xl font-bold text-gray-900 mt-1">{pendingCount}</p></div>
            <div className="p-2.5 rounded-lg bg-orange-500/10 text-orange-600"><Clock className="w-5 h-5" /></div>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-gray-600 text-sm">Expired</p><p className="text-2xl font-bold text-gray-900 mt-1">{subs.filter((s) => s.status === 'expired' || s.status === 'cancelled').length}</p></div>
      ...
      {pendingCount > 0 && (
        <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => setActiveTab('pending')} className="ml-auto text-sm text-orange-600 hover:text-orange-300 font-medium">
              View
            </button>
          </div>
        </div>
      ...
      <div className="flex gap-2 mb-6">
        <button type="button" onClick={() => setActiveTab('all')}
          className={`px-4 py-2 rounded-lg font-medium transition text-sm ${activeTab === 'all' ? 'bg-[#6B3FD9] text-white' : 'bg-white text-gray-600 hover:text-white border border-gray-200'}`}>
          All
        </button>
        <button type="button" onClick={() => setActiveTab('pending')}
          className={`px-4 py-2 rounded-lg font-medium transition text-sm flex items-center gap-2 ${activeTab === 'pending' ? 'bg-[#6B3FD9] text-white' : 'bg-white text-gray-600 hover:text-white border border-gray-200'}`}>
          Pending
          {pendingCount > 0 && (
        ...
        <button type="button" onClick={() => setActiveTab('ledger')}
          className={`px-4 py-2 rounded-lg font-medium transition text-sm flex items-center gap-2 ${activeTab === 'ledger' ? 'bg-[#6B3FD9] text-white' : 'bg-white text-gray-600 hover:text-white border border-gray-200'}`}>
                ...
                <tbody>
                  {filtered.map((sub) => (
                    <tr key={sub.id} className="border-b border-gray-200 last:border-0 hover:bg-gray-200 transition">
                      ...
                      <td className="px-6 py-4">
                        <span className={`text-xs px-2.5 py-1 rounded-lg ${statusStyles[sub.status] ?? 'bg-gray-500/10 text-gray-600'}`}>
                        ...
                        {sub.status === 'pending_approval' && (
                          <div className="flex items-center gap-2">
                            <button type="button" onClick={() => approve(sub)} disabled={actionId === sub.id} title="Approve"
                              className="p-2 rounded-lg text-green-600 hover:bg-green-500/10 transition disabled:opacity-50">
                            ...
                            <button type="button" onClick={() => reject(sub)} disabled={actionId === sub.id} title="Reject"
                              className="p-2 rounded-lg text-red-600 hover:bg-red-500/10 transition disabled:opacity-50">
                              <X className="w-4 h-4" />
                            </button>
                          </div>
                        )}
            ...
            {filtered.map((sub) => (
              <div key={sub.id} className="bg-white rounded-xl border border-gray-200 p-4">
                <div className="flex items-start justify-between mb-3">
                  <p className="font-medium text-gray-900">{sub.shop_name}</p>
                  <span className={`text-xs px-2.5 py-1 rounded-lg ${statusStyles[sub.status] ?? 'bg-gray-500/10 text-gray-600'}`}>
                    {statusLabel[sub.status] ?? sub.status}
                  </span>
                </div>
                ...
                {sub.status === 'pending_approval' && (
                  <div className="flex gap-2 pt-3 border-t border-gray-200">
                    <button type="button" onClick={() => approve(sub)} disabled={actionId === sub.id}
                      className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-green-500/10 text-green-600 hover:bg-green-500/20 transition font-medium text-sm disabled:opacity-50">
                    ...
                    <button type="button" onClick={() => reject(sub)} disabled={actionId === sub.id}
                      className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg bg-red-500/10 text-red-600 hover:bg-red-500/20 transition font-medium text-sm disabled:opacity-50">
    ...
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <p className="text-2xl font-bold text-gray-900 mt-1">{ledger.length}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-gray-600 text-sm flex items-center gap-1.5"><Zap className="w-3.5 h-3.5 text-yellow-600" /> Lemon Squeezy</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">{totalLemonSqueezy.toLocaleString()}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <p className="text-gray-600 text-sm flex items-center gap-1.5"><UserCheck className="w-3.5 h-3.5 text-gray-600" /> Manual</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">{totalManual.toLocaleString()}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-600">No confirmed payments yet</p>
        </div>
      ) : (
                    ...
                    <td className="px-6 py-4">
                      {p.commission ? (
                        <div className="text-sm">
                          <span className={p.commission.status === 'reversed' ? 'text-gray-500 line-through' : 'text-gray-900 font-medium'}>
                            ${p.commission.amount?.toFixed(2)}
                          </span>
                          <span className="text-gray-500"> → {p.commission.affiliate_name}</span>
                          {p.commission.type === 'recurring' && (
                            <span className="ml-1.5 text-xs text-purple-600">Month {p.commission.period_month}/12</span>
                          )}
                          {p.commission.status === 'reversed' && (
                            <span className="ml-1.5 text-xs text-red-600 font-medium">Reversed</span>
                          )}
                        </div>
                      ) : (
                    ...
                    <td className="px-6 py-4">
                      {p.refunded_at ? (
                        ...
                        <button type="button" onClick={() => handleRefund(p)} disabled={refundingId === p.id}
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-500/10 text-red-600 hover:bg-red-500/20 transition text-xs font-medium disabled:opacity-50">
