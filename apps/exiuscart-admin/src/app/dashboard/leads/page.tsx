...
const statusStyles: Record<string, string> = {
  ...
  demo: 'bg-purple-500/10 text-purple-600',
      ...
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-6">
        {STATUS_OPTIONS.map((s) => (
          <div key={s} className="bg-white rounded-xl border border-gray-200 p-3 text-center">
            <p className="text-2xl font-bold text-gray-900">{leads.filter((l) => l.status === s).length}</p>
            <p className="text-xs text-gray-600 mt-1 capitalize">{s}</p>
          </div>
        ))}
      </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {lead.shop_name && <p className="text-sm text-gray-600">· {lead.shop_name}</p>}
                    {lead.business_type && (
                      <span className="text-xs bg-gray-200 text-gray-700 px-2 py-0.5 rounded">{lead.business_type}</span>
                    )}
                  </div>
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  <span className={`text-xs px-2.5 py-1 rounded-lg capitalize ${statusStyles[lead.status] ?? 'bg-gray-500/10 text-gray-600'}`}>
                    {lead.status}
                  </span>
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl border border-gray-200 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-gray-200 sticky top-0 bg-white">
          <h2 className="text-lg font-semibold text-gray-900">{lead ? 'Edit Lead' : 'Add Lead'}</h2>
          <button type="button" onClick={onClose} className="p-2 hover:bg-gray-100 rounded-lg text-gray-600 hover:text-gray-900 transition">
            <X className="w-5 h-5" />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          {error && <div className="bg-red-500/10 border border-red-500/20 text-red-600 text-sm rounded-lg px-4 py-3">{error}</div>}

          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="text-sm text-gray-600 mb-1.5 block">Name *</label>
              <input type="text" value={form.name} onChange={(e) => set('name', e.target.value)} required
                className="w-full px-3 py-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:border-[#6B3FD9] outline-none text-gray-900" />
            </div>
            <div>
              <label className="text-sm text-gray-600 mb-1.5 block">Store Name</label>
              <input type="text" value={form.shop_name} onChange={(e) => set('shop_name', e.target.value)}
                className="w-full px-3 py-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:border-[#6B3FD9] outline-none text-gray-900" />
            </div>
            <div>
              <label className="text-sm text-gray-600 mb-1.5 block">Business Type</label>
            ...
            <div>
              <label className="text-sm text-gray-600 mb-1.5 block">Source</label>
            ...
            <div className="col-span-2">
              ...
              <textarea value={form.notes} onChange={(e) => set('notes', e.target.value)} rows={3}
                className="w-full px-3 py-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:border-[#6B3FD9] outline-none text-gray-900 resize-none" />
            </div>
          </div>

          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} disabled={saving}
              className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-gray-900 hover:bg-gray-100 transition">
              Cancel
            </button>
            <button type="submit" disabled={saving}
              className="flex-1 px-4 py-2.5 bg-[#6B3FD9] text-white font-semibold rounded-lg hover:bg-[#5A2EC9] transition disabled:opacity-50 inline-flex items-center justify-center gap-2">
