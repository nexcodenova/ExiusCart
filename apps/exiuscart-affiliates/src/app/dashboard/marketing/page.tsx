...
export default function MarketingPage() {
  ...
  return (
    <div>
      ...
      {/* Referral link — only shown when code is available (active affiliate) */}
      {affiliateCode ? (
        <div className="bg-[#151F32] border border-gray-800 rounded-2xl p-6 mb-6">
          ...
          <div className="mt-3 flex items-center gap-4 text-xs text-gray-500">
            <span>Your code: <span className="text-white font-mono font-bold">{affiliateCode}</span></span>
            <span>· Tracking cookie: 30 days</span>
          </div>
        </div>
      ) : (
        <div className="bg-[#151F32] border border-gray-800 rounded-2xl p-6 mb-6 text-center text-gray-500 text-sm">
          Referral link unavailable — your account may not be active yet.
        </div>
      )}

      {/* Copy templates — only shown when code is active */}
      {copyTexts.length > 0 && (
        <>
